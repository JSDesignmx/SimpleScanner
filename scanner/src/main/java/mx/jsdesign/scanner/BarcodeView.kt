...
import mx.jsdesign.scanner.overlay.BarcodeOverlay
import mx.jsdesign.scanner.overlay.BarcodeRectOverlay
import mx.jsdesign.scanner.overlay.Optional
import mx.jsdesign.scanner.scanner.BarcodeScanner
import mx.jsdesign.scanner.scanner.BarcodeScannerConfig
import mx.jsdesign.scanner.scanner.Camera
import mx.jsdesign.scanner.scanner.Size
...
class BarcodeView : FrameLayout {
    ...
    private val xScaleFactorL by lazy {z
    ...
    private fun onSurfaceReady(emitter: ObservableEmitter<Boolean>) {
        if (!emitter.isDisposed) {
            if (!checkPermission()) {
                emitter.onError(SecurityException("No se tiene acceso a la camara"))
            } else {
                try {
                    setCameraSettings()
                } catch (e: Exception) {
